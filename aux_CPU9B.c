...
int main()
{
  ...
  for (int tz = 0; tz < 8; tz ++) {
    ...
    //120214    printf("#define B%d_COMPXYZ%d() ", B, tz);
    printf("#define B%d_LOADXYZ%d() ", B, tz);
    ...
    //120214    /* Initialise offsets */
    //120214    int Mjoff = 0;
    //120214    int Kijoff = 0;

    /* Initialise the counter for sources */
    int isrc = 0;
	...
	for (int fdx = 0; fdx < 6; fdx ++) {
	  ...
	  if (abs(kx) > 1 || abs(ky) > 1 || abs(kz) > 1) {
	    ...
	    //120214	    int Mjoff_old = Mjoff;
	    //120214	    int Kijoff_old = Kijoff;
	    //120214	    Mjoff = ((sib * (B + 2) + iz) * (B + 2) + iy) * (B + 2) + ix; // Mj[8][B+2][B+2][B+2]
	    //120214	    Kijoff = Ktable[(kx + 3) + 7 * (ky + 3) + 49 * (kz + 3)];
	    //120214
	    //120214	    printf("COMP(%d, %d)", Kijoff - Kijoff_old, Mjoff - Mjoff_old);

	    int Mjoff = ((sib * (B + 2) + iz) * (B + 2) + iy) * (B + 2) + ix; // Mj[8][B+2][B+2][B+2]
	    int Kijoff = Ktable[(kx + 3) + 7 * (ky + 3) + 49 * (kz + 3)];
	    
	    printf("LD(%d, %d, %d)", isrc, Kijoff, Mjoff);
	    isrc ++;
