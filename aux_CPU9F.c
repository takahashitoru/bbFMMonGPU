  ...
  for (int tz = 0; tz < 8; tz ++) {
	...
	for (int fdx = 0; fdx < 6; fdx ++) {
	  ...
	  if (abs(kx) > 1 || abs(ky) > 1 || abs(kz) > 1) {
	    ...
	    //120218	    /* Obtain the source's sibling-index */
	    //120218	    int sib = 4 * (fdx % 2) + 2 * (fdy % 2) + (fdz % 2);
	    //120218
	    //120218	    /* Obtain the source's coordinates in 6x6x6 region (except
	    //120218	       for the offset due to obs's sibling-index) */
	    //120218	    int ix = fdx / 2; // 0<=ix<3
	    //120218	    int iy = fdy / 2; // 0<=iy<3
	    //120218	    int iz = fdz / 2; // 0<=iz<3

	    int Mjoff_old = Mjoff;
	    int Kijoff_old = Kijoff;
	    //120218	    Mjoff = ((sib * (B + 2) + iz) * (B + 2) + iy) * (B + 2) + ix; // Mj[8][B+2][B+2][B+2]
