...
//#include "cuda.h" // CUDA device driver (cuInit() etc)

#include "cuopts.cu"
#include "eventTimer.cu"
#include "anotherFMMCompute.cu"

#include "myDeviceQuery.cu"
...
int main(int argc, char *argv[]) 
{
  ...
  /* Initialize CUDA (this is not absolutely necessary, but important
     for timing) */
#ifndef DISABLE_TIMING
  allocTimer(&timer_cuda);
  initTimer(timer_cuda);
  startTimer(timer_cuda);
#endif
  cudaFree(0);
  cudaEvent_t time_dummy;
  cudaEventCreate(&time_dummy);
#ifndef DISABLE_TIMING
  stopTimer(timer_cuda);
#endif
...
#if defined(DEVICE_QUERY) || defined(_DEBUG) || defined(MYDEBUG)
  myDeviceQuery();
#endif
...
#ifndef DISABLE_TIMING
  stopTimer(timer_cuda);
  printTimer(stderr, "cuda", timer_cuda);
  freeTimer(&timer_cuda);
#endif
